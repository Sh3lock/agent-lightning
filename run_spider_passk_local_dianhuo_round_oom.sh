#!/bin/bash
# 一键启动 Spider Pass@k 训练（OOM 48条 + 点火版：Round-based guidance）
# 
# 简化方案：guidance 嵌入 prompt 模板中间，无需 v0 proxy
# - 使用 v1 路径（VLLM_USE_V1=1）
# - guidance 通过 trainer.py 退火逻辑控制
# - 不再需要 suffix 分段和 truncate_prompt_tokens 设置
...
# 这是你坚持要存放文件的真实长路径
REAL_RAY_DIR="/home/storage/wenbinxing/ltf/tmp/raytmp_oom/ray_$TS"
mkdir -p "$REAL_RAY_DIR"

# 这是给 Ray 用的“短马甲”链接（放在 /tmp 下，名字极短）
SHORT_LINK="/tmp/r_${MY_USER}_oom"
...
# 2. 核心参数设置
STAGE=1
CONFIG_STAGE1="configs/passk_stage1_qwen05b_2epochs_oom.json"
...
# [注意] 这里目前指定了 GPU 1，如果需要更改请手动修改下面的数字
# 前台运行，输出直接显示在终端（关键指标会通过 wandb 记录）
CUDA_VISIBLE_DEVICES=1 python train_sql_agent.py local_qwen05 \
